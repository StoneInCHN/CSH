...
@Entity
@Table(name = "csh_product", indexes = {@javax.persistence.Index(name = "product_tenantid",
    columnList = "tenantID")})
...
public class Product extends BaseEntity {
   ...
   * 设置商品促销失败原因
   * 
   * @param promotionFailReason 商品促销失败原因
   */
   ...
   * 设置促销商品全称
   * 
   * @param promotionFullName 促销商品全称
   */
   ...
   * 设置促销返点率
   * 
   * @param promotionRebateRate 促销返点率
   */
...
/**
  ...
  public void setAttributeValue(Attribute attribute, String value) {
    if (attribute != null && attribute.getPropertyIndex() != null) {
      ...
      if (value == null
          || (attribute.getOptions() != null && attribute.getOptions().contains(value))) {
        try {
          String propertyName = ATTRIBUTE_VALUE_PROPERTY_NAME_PREFIX + attribute.getPropertyIndex();
  ...
  public Boolean getIsOutOfStock() {
    if (getStockAlertCount() != null) {
      return getStock() != null && getAllocatedStock() != null
          && getAllocatedStock() + getStockAlertCount() >= getStock();
    } else {
