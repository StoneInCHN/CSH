...
/**
 * 设备查询控制器 Created by zhangye on 2016/12/23.
 */
...
public class DeviceInfoController extends MobileBaseController {
  ...
  public @ResponseBody BaseResponse bindDevice(@RequestBody DeviceInfoRequest request) {
    ...
    if (LogUtil.isDebugEnabled(getClass())) {
      LogUtil.debug(getClass(), "bindDevice", "bind success:deviceId = %s, vehicleId = %s",
          deviceId, vehicleId);
    }
  ...
  public @ResponseBody DeviceDetailResponse getDeviceDetail(@RequestBody DeviceInfoRequest request) {
    ...
    DeviceInfo deviceInfo = deviceInfoService.find(deviceId);
    String[] properties =
        {"id", "deviceNo", "type", "bindStatus", "deviceStatus", "vehicle.plate", "vehicle.id",
            "bindTime", "createDate"};
    ...
    if (msg != null) {
      ...
      Map<String, Object> xy =
          LatLonUtil.convertCoordinate(((Double) msg.get("lon")).toString(),
              ((Double) msg.get("lat")).toString());
      resultMap.putAll(xy);
      // 方位角
    ...
    if (LogUtil.isDebugEnabled(getClass())) {
      LogUtil.debug(getClass(), "getDeviceDetail", "response success:deviceId = %s, deviceNo = %s",
          deviceId, deviceNo);
    }
  ...
  public @ResponseBody DevicePageResponse findPage(@RequestBody DeviceInfoRequest request) {
    ...
    String[] properties =
        {"id", "deviceNo", "type", "bindStatus", "deviceStatus", "vehicle.plate", "bindTime",
            "createDate"};
    List<Map<String, Object>> resultMaps =
        FieldFilterUtils.filterCollectionMap(properties, pageByRequest.getContent());
    ...
    // 设备数量
    Integer bindDeviceCount =
        deviceInfoService.countByBindStatus(tenantId, CommonEnum.BindStatus.BINDED);
    Integer unbindDeviceCount =
        deviceInfoService.countByBindStatus(tenantId, CommonEnum.BindStatus.UNBINDED);
