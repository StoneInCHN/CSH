...
<input type="hidden" id="orderDirection" name="orderDirection" value="${page.orderDirection}" />
	<div class="pagination">
		...
		[/#if]
		<span class="pageSkip">
			${message("asup.page.totalPages", totalPages)} ${message("asup.page.pageNumber", '<input id="pageNumber" name="pageNumber" value="' + pageNumber + '" maxlength="9" onpaste="return false;" />')}<button type="submit">&nbsp;</button>
		</span>
	</div>
