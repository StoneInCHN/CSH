...
<head>
<title>${message("csh.deviceInfo.batchAdd")}</title>
                     ...
                     <form id="inputForm" action="batchAddSave.jhtml" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
                     	<table class="input tabContent">
							<tr>
								...
								<td>
									<input type="button" class="button" value="${message("csh.common.downloadTemplate")}" onclick="location.href='${base}/template/template.xlsx'" />
								</td>
							</tr>
							<tr>
								<th>
									<span class="requiredField">*</span>${message("csh.deviceInfo.batch.choosefile")}:
								</th>
								<td>
							...
							<tr id="statusTr" class="hidden">
								...
								<td>
									...
									<div id="status">${message("csh.deviceInfo.batch.upload.processing")}</div>
								</td>
							</tr>
							[#if resMessage.content??]
							<tr>
								<th>${message("csh.deviceInfo.upload.result")}</th>
								
								<td>
									<table>
										<tr>
											<td>${message("csh.deviceInfo.upload.success")}</td>
											<td>${successCount}</td>
										</tr>
										<tr>
											<td>${message("csh.deviceInfo.upload.fail")}</td>
											<td>${faileCount}</td>
										</tr>
										[#if faileCount > 0]
											[#if dupfailDeviceIds?? && dupfailDeviceIds?size>0]
											<tr>
												<td>${message("csh.deviceInfo.upload.failimei")}</td>
												<td>${dupfailDeviceIds}</td>
											</tr>
											[/#if]
											[#if formatFailDeviceIds?? && formatFailDeviceIds?size>0]
												<tr>
												<td>${message("csh.deviceInfo.upload.failformat")}</td>
