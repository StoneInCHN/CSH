...
if (applicationContext != null) {
	...
	if (loginFailure != null) {
		if (loginFailure.equals("org.apache.shiro.authc.pam.UnsupportedTokenException")) {
			message = "asup.captcha.invalid";
		} else if (loginFailure.equals("org.apache.shiro.authc.UnknownAccountException")) {
			message = "asup.login.unknownAccount";
		} else if (loginFailure.equals("org.apache.shiro.authc.DisabledAccountException")) {
			message = "asup.login.disabledAccount";
		} else if (loginFailure.equals("org.apache.shiro.authc.LockedAccountException")) {
			message = "asup.login.lockedAccount";
		} else if (loginFailure.equals("org.apache.shiro.authc.IncorrectCredentialsException")) {
				message = "asup.login.incorrectCredentials";
		} else if (loginFailure.equals("org.apache.shiro.authc.AuthenticationException")) {
			message = "asup.login.authentication";
		}else{
			message = "asup.login.incorrectCredentials";
		}
	}
%>
<title><%=SpringUtils.getMessage("asup.login.title")%> </title>
...
<script type="text/javascript">
	$().ready( function() {
		...
		$loginForm.submit( function() {
			if ($username.val() == "") {
				$.message("warn", "<%=SpringUtils.getMessage("asup.login.usernameRequired")%>");
				return false;
			}
			if ($password.val() == "") {
				$.message("warn", "<%=SpringUtils.getMessage("asup.login.passwordRequired")%>");
				return false;
			}
			if ($captcha.val() == "") {
				$.message("warn", "<%=SpringUtils.getMessage("asup.login.captchaRequired")%>");
				return false;
			}
...
<body >
	...
	<div class="warp">
				...
				<form id="loginForm" action="login.jsp" method="post">
					...
					<%}%>
						...
						<div class="div_input">
									...
									<img id="captchaImage" class="captchaImage" src="<%=base%>/console/common/captcha.jhtml?captchaId=<%=captchaId%>" title="<%=SpringUtils.getMessage("asup.captcha.imageTitle")%>" />
						</div>
						<%}%>
						<div class="div_input">
							<input type="checkbox" class="checkbox" id="isRememberUsername" value="true" /><%=SpringUtils.getMessage("asup.login.rememberUsername")%>
							<a href="<%=base%>/console/password/forgetPwd.jhtml" class="forgetLink" target="_blank" style="color:red"><%=SpringUtils.getMessage("asup.forget.password")%></a>
						</div>
						<div class="div_input">
								<input type="submit" class="loginButton" value="<%=SpringUtils.getMessage("asup.login.login")%>" />
						</div>
						<div class="div_input">
		                	 <span>还没有账号？<a href="<%=base%>/reg.jsp" class="registerLink">立刻申请</a></span>
		               </div>
				</form>
			    </div>
			</div>
		...
		<div class="footer-warp">
		    <div class="copy-warp">
		        <div class="copy">版权所有©2014<span>Copyright © 2014  rights reserved.</span></div>
		    </div>
		</div>
<script type="text/javascript">
    $(function(){
