...
//		Filter brandDetailFilter = new Filter("vehicleBrandDetails", Operator.eq, vehicleBrandDetails);
...
//	    	List<ItemPart> itemParts = itemPartDao.findList(null, null, filters, null);
	    	List<ItemPart> itemParts = itemPartDao.getItemParts(item, vehicleBrandDetails, vehicleLines, vehicleBrands);
	    	if (itemParts!=null && itemParts.size()>0) {
	    		itemMap.put("serviceItemName", item.getServiceItemName());
	    		String[] properties =
	    	              {"id", "serviceItemPartName", "price"	};
