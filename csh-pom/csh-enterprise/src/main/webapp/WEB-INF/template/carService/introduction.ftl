...
<body>
	<p class="top">
	<span class="time">${carService.createDate}</span> 
	<span class="hit">评分:${carService.rate}</span>
	<span class="hit">购买次数:${carService.purchaseCounts}</span>
	</p>
	<div>
		<h3 style="text-align:center;margin:30px">${carService.name}</h3>
		<div style="margin-bottom:80px;">${carService.serviceDesc}</div>
	</div>
</body>
</html>