...
<body>
	<p class="top"><span class="category" >${product.fullName}</span><span class="time">${product.createDate}</span> <span class="hits">点击数:${product.hits}</span></p>
	<div>
		<h3 style="text-align:center;margin:30px">${product.name}</h3>
		<div style="margin-bottom:80px;">${product.introduction}</div>
	</div>
</body>
</html>