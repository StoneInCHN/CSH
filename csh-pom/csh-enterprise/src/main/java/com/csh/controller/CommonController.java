...
public class CommonController extends BaseController {
  ...
  @Resource(name = "tenantMsgServiceImpl")
  private TenantMsgService tenantMsgService;
  ...
  public void image(String captchaId, String clientSessionId, String timestamp,
    ...
    if (LogUtil.isDebugEnabled(getClass())) {
      LogUtil.debug(getClass(), "image", "Parameter captchaId=%s, "
          + "session Id=%s, clientSessionId=%s, timestamp=%s", captchaId, session.getId(),
  ...
  public String main(ModelMap model, HttpSession session) {
    ...
    TenantInfo tenantInfo =
        tenantInfoService.find(tenantAccountService.getCurrentTenantInfo().getId());
  ...
  public @ResponseBody Message saveTenantInfoConfig(TenantInfo tenantInfo,
    ...
    tenantInfoService.update(tenantInfo, "tenantName", "ownerName", "address", "createDate",
        "orgCode", "accountStatus", "versionConfig", "area", "praiseRate", "isHaveAccount",
        "distributor", "qrImage");
    return SUCCESS_MESSAGE;
  }
  ...
  public @ResponseBody Message refreshLuceneIndex(HttpServletRequest request) {
  ...
  public @ResponseBody Message generateQrImage(HttpServletRequest request) {

    Long tenantID = tenantAccountService.getCurrentTenantID();
    String content =
        "{\"flag\":\"" + DigestUtils.md5Hex("车生活") + "\",\"tenantInfo\":\"" + tenantID + "\"}";
  ...
  public void getQrCode(HttpServletResponse response) {
    ...
    try {
      if (null != tenantInfo.getQrImage()) {
        InputStream buffin = new ByteArrayInputStream(tenantInfo.getQrImage());
