...
public class ItemPartServiceImpl extends BaseServiceImpl<ItemPart,Long> implements ItemPartService {
          ...
          Long[] deleteBrandDetailItemIds, Long[] selectBrandDetailItemIds)
      {
        ...
        Set<VehicleBrandDetail> origVehicleBrandDetailSet = origItemPart.getVehicleBrandDetails ();
        ...
        //删除的车品牌
        List<VehicleBrand> deleteBrandList = vehicleBrandService.findList (deleteBrandItemIds);
        ...
        //添加的车品牌
        List<VehicleBrand> selectBrandList = vehicleBrandService.findList (selectBrandItemIds);
        ...
        //删除的车系
        List<VehicleLine> deleteLineParentList = vehicleLineService.findList (deleteLineParentItemIds);
        ...
        //添加的车系
        List<VehicleLine> selectLineParentList = vehicleLineService.findList (selectLineParentItemIds);
        ...
        //删除的车系
        List<VehicleLine> deleteLineChildList = vehicleLineService.findList (deleteLineChildItemIds);
        ...
        for (VehicleLine deleteLine : deleteLineChildList)
        {
          ...
          {
            newVehicleLineSet.remove (deleteLine);
            deleteVehicleBrandDetailUnderLine.addAll (deleteLine.getBrandDetails ());
          }
        }
        //添加的车系
        List<VehicleLine> selectLineChildList = vehicleLineService.findList (selectLineChildItemIds);
        ...
        //删除的车型
        List<VehicleBrandDetail> deleteBrandDetailList = vehicleBrandDetailService.findList (deleteBrandDetailItemIds);
        ...
        //添加的车型
        List<VehicleBrandDetail> selectBrandDetailList = vehicleBrandDetailService.findList (selectBrandDetailItemIds);
        ...
        if (itemPart.getIsDefault ())
        {
          List<Filter> filters = new ArrayList<Filter>();
          Filter isDefault = new Filter ("isDefault", Operator.eq, true);
          Filter carServiceItemFilter = new Filter ("carServiceItem", Operator.eq, origItemPart.getCarServiceItem ());
          filters.add (isDefault);
          filters.add (carServiceItemFilter);
